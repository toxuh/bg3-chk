    ...
    <aside className="sticky top-[137px] hidden self-start overflow-hidden rounded-xl border border-stone-800 bg-stone-900 shadow-lg shadow-black/10 lg:block">
      <div className="flex items-center gap-3 border-b border-stone-800 px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-amber-500">{t("title")}</p>
          <p className="truncate text-xs text-stone-500">{t("description")}</p>
        </div>
        <Button
          asChild
          variant="ghost"
        ...
        >
          <a href={mapUrl} target="_blank" rel="noreferrer" aria-label={t("openInNewTab")}>
            <ExternalLink />
          </a>
        </Button>
      </div>
      <iframe title={t("iframeTitle")} src={mapUrl} className="h-[660px] w-full bg-stone-950" />
    </aside>
  );
};
...
export const ChecklistMobileMap = ({ mapUrl, onClose }: Readonly<MobileProps>) => {
  ...
  return (
    <div className="fixed inset-x-3 bottom-3 top-24 z-40 overflow-hidden rounded-xl border border-stone-700 bg-stone-900 shadow-2xl lg:hidden">
      ...
      <iframe title={t("iframeTitle")} src={mapUrl} className="h-[calc(100%-49px)] w-full bg-stone-950" />
    </div>
  );
};
